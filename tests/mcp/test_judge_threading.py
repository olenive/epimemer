...
class TestTheLifecycleEpisodeCarriesWhoRetiredIt:
    ...
    async def test_supersede_by_an_existing_node_names_its_judge(self, storage, embedder):
    ...
    async def test_retiring_without_a_judge_is_a_normal_write(self, storage, embedder):
...
class TestRetiringAndReturningAreTwoDecisions:
    ...
    async def test_the_reactivation_edge_names_the_judge_too(self, storage, embedder):
        from epimemer.core.types import RawDocument
...
class TestTheEdgeCarriesWhoAssertedIt:
    ...
    async def test_both_similarity_verdicts_name_their_judge(self, storage, embedder):
    ...
    async def test_re_recording_a_pair_does_not_restamp_it(self, storage, embedder):
...
class TestTheValueSignalCarriesWhoJudgedImportance:
    async def test_the_latest_judgment_names_its_judge(self, storage, embedder):
        ...
        await tools.judge_importance(
            node.id,
            direction="up",
            reason="cited in the postmortem",
            storage=storage,
            judge=CRITIC,
        )
    ...
    async def test_every_entry_in_the_trail_names_its_own_judge(self, storage, embedder):
        ...
        assert [(e.get("judged_by") or {}).get("agent_id") for e in trail] == [
            "critic",
            "editor",
            None,
        ]
...
class TestTheNodeCarriesWhoWroteItsWording:
    ...
    async def test_a_correction_credits_the_agent_that_wrote_it(self, storage, embedder):
        node = await _fact(storage, embedder, "the office is in Leeds")
    ...
    async def test_a_synthesised_parent_names_its_judge(self, storage, embedder):
        ...
        parents = [
            n
            for n in await storage.query_nodes(node_type=None)
...
class TestWhatTheAgentSees:
    async def test_an_unknown_judge_is_not_sent_back_as_null(self, storage, embedder):
...
class TestBothBackendsKeepIt:
    ...
    async def test_an_archived_node_carries_its_judges_through_the_blob(self, storage, embedder):
