"""Declaring the metacontext of a graph written before metacontexts were required.
...
It exists because absence stopped meaning anything. A node carrying no metacontext
shares a metacontext with nothing: never compared, never merged, absent from every
scoped search. Before the requirement that state was the whole graph, and
`graph_stats.nodes_without_metacontext` is how a user watches it reach zero.
"""
...
async def _node(storage, node, *, metacontexts=()):
...
class TestTheSweepStampsWhatNobodySpokeFor:
    async def test_a_node_without_a_metacontext_is_declared(self, storage):
        legacy = await _node(storage, Topic(content="Vienna", source_id="s1"))

        result = await declare_metacontext(storage, metacontext=BASE_METACONTEXT_ID, judge=DECLARER)

        assert result.declared == 1
        assert await _metacontexts_of(storage, legacy.id) == {BASE_METACONTEXT_ID}

    async def test_a_node_already_in_one_is_left_alone(self, storage):
        """The predicate is *no metacontexts at all*, never *not this metacontext*. A node
        already in a fiction metacontext must not acquire a second one from a sweep
        ...
        assert result.already_in_metacontext == 1
        assert await _metacontexts_of(storage, placed.id) == {fiction["metacontext_id"]}

    async def test_a_rerun_declares_nothing(self, storage):
        """Idempotent, and that is what lets the command be run without
        checking first — and what makes it naturally dead once no undeclared
        ...
        again = await declare_metacontext(storage, metacontext=BASE_METACONTEXT_ID, judge=DECLARER)

        assert again.declared == 0
        assert again.already_in_metacontext == 1

    async def test_a_metacontext_that_does_not_exist_is_refused(self, storage):
        """A sweep that minted the metacontext it was about to stamp would do the one
    ...
    async def test_an_empty_graph_is_not_an_error(self, storage):
        result = await declare_metacontext(storage, metacontext=BASE_METACONTEXT_ID)
        assert result.declared == 0
        assert result.already_in_metacontext == 0
    ...
    async def test_the_edges_carry_the_declaring_judge(self, storage):
        ...
        ambiguity the metacontext requirement exists to end."""
...
class TestOneRowForTheSweep:
    async def test_the_sweep_journals_once_naming_what_it_stamped(self, storage):
        ...
        rows = await storage.query_decisions(kinds=[DecisionKind.METACONTEXT_DECLARATION])
        assert len(rows) == 1
    ...
    async def test_a_sweep_that_stamped_nothing_journals_nothing(self, storage):
        ...
        assert await storage.query_decisions(kinds=[DecisionKind.METACONTEXT_DECLARATION]) == []


class TestTheQuarantineMetacontextIsWriteOnlyForTheSweep:
    async def test_the_sweep_may_stamp_it(self, storage):
        """For a graph nobody can vouch for: the nodes are marked as exactly
        that, rather than left carrying no metacontext — which would leave them
        ...
        await declare_metacontext(storage, metacontext=QUARANTINE_METACONTEXT_ID, judge=DECLARER)

        assert await _metacontexts_of(storage, legacy.id) == {QUARANTINE_METACONTEXT_ID}

    async def test_an_agent_may_not_ingest_into_it(self, storage, embedder, config):
        """A metacontext an agent can assert into stops meaning *nobody vouched for
        ...
        with pytest.raises(ValueError, match="not a metacontext anything may be"):
...
class TestGraphStatsIsTheCompletenessCheck:
    async def test_it_counts_what_names_no_metacontext(self, storage):
        await _node(storage, Topic(content="Vienna", source_id="s1"))
    ...
    async def test_a_node_already_in_one_never_counts(self, storage):
        fiction, _ = await tools.create_metacontext("The novel", storage)
        await _node(
            storage,
            Topic(content="the council", source_id="s1"),
