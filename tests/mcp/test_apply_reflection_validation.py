...
async def _twin_facts(storage, embedding_provider) -> tuple[Fact, Fact]:
    ...
    facts = [
        Fact(content="Ada Lovelace wrote the first algorithm.",
             source_id="doc-a", claim_kind=ClaimKind.EVENT),
        Fact(content="The first algorithm was written by Ada Lovelace.",
             source_id="doc-b", claim_kind=ClaimKind.EVENT),
    ]
    for fact in facts:
        await storage.store_node(fact)
        await storage.store_embedding(EmbeddingRecord(
            item_id=fact.id, model_id=embedding_provider.model_id, vector=_TWIN,
        ))
...
class TestNothingIsWrittenBeforeTheBatchIsChecked:
    ...
    async def test_the_permanent_suppression_is_named(
    ...
    async def test_a_later_step_cannot_abort_an_earlier_one(
        self, storage, embedding_provider
    ):
        ...
        with pytest.raises(ValueError):
            await tools.apply_reflection(
                ...
                similarities=[{
                    "pair": [first.id, second.id],
                    "verdict": "distinct", "because": "different claims",
                }],
                boundaries=[{"node_id": first.id, "source_id": "doc-a",
                             "endpoint": "end"}],   # no `at`
            )
        assert await _similarity_rows(storage) == 0

    async def test_every_problem_is_listed_at_once(
        self, storage, embedding_provider
    ):
...
class TestRefusableJudgmentsStillCostOneEntry:
    ...
    async def test_a_missing_kind_refuses_only_its_own_entry(
        ...
        result, _ = await tools.apply_reflection(
            ...
            relation_verdicts=[{
                ...
                "verdict": "distinct", "because": "different relations",
    ...
    async def test_an_unknown_node_id_is_still_skipped(
        self, storage, embedding_provider
    ):
...
class TestWhatCountsAsMalformed:
    ...
    def test_a_pair_that_does_not_name_two(self):
        found = malformed_entries({
            "similarities": [{"pair": ["only-one"], "verdict": "distinct"}]
        })
        assert found[0].problem == "'pair' names 1 thing(s); a verdict is about two"

    def test_an_unknown_supersession_reason(self):
        found = malformed_entries({"supersessions": [
            {"old_id": "a", "by_id": "b", "because": "i_felt_like_it"}
        ]})
    ...
    def test_a_boundary_date_that_will_not_parse(self):
        found = malformed_entries({"boundaries": [{
            "node_id": "n", "source_id": "s", "endpoint": "end", "at": "last tuesday",
        }]})
        assert "neither a datetime nor an ISO-8601 string" in found[0].problem

    def test_a_boundary_date_that_will(self):
        for at in ("2026-08-28T00:00:00+00:00", datetime.now(timezone.utc)):
            assert malformed_entries({"boundaries": [{
                "node_id": "n", "source_id": "s", "endpoint": "end", "at": at,
            }]}) == []
    ...
    def test_the_index_identifies_the_entry(self):
        ...
        found = malformed_entries({"splits": [
...
def _sample_entry(field: str) -> dict:
    ...
    samples = {
        ...
        "boundaries": {"node_id": "n", "source_id": "s", "endpoint": "end",
...
def _required_subscripts() -> dict[str, set[str]]:
    ...
    for loop in (n for n in ast.walk(fn) if isinstance(n, ast.For)):
        ...
        if not (isinstance(loop.iter, ast.BoolOp)
