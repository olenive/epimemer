...
| **next** | `REVIEW_MODE.md` step 7, or 73 | Step 7 is `by_agent` / `since` / `unreviewed` / `advisory`, `certainty_ceiling`, and `apply_review` — the first writer of a `certainty`, and what makes a confirmation cost something. **73** (the cross-graph decision locator) is independent and unblocked, and a field on `review()`'s response is where it goes. Neither is urgent: the journal is one row old on the real graphs, so both get more useful the longer they wait |
