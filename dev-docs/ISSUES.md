...
### Issue 69 — merging relation labels leaves no record of who did it — 🟢 RESOLVED 2026-08-27 by #74 stage 3 (found 2026-08-23)

> **Resolved 2026-08-27, and by dissolution rather than by picking one of the
> four shapes below.** #74 stage 3 ships `DecisionKind.RELATION_VERDICT` with
> `subject_ids = [label_a.id, label_b.id]` — **ids of records that exist in this
> graph**, so no field acquires a second namespace, no subject list runs to the
> hundreds, and nothing surfaces under a node the decision was not about. The
> question was unanswerable for exactly one reason, and this entry never named
> it: *the subject had no identity*. Stage 1 gave it one.
>
> Two things this does **not** close. `relation_merges` itself still journals
> nothing — the subjects are now nameable, and no writer has been built, which
> waits on §5's undecided question of whether label merging survives at all; if
> it does, `relation_merge` is stage 4's kind, and `DecisionKind`'s docstring now
> points at `RELATION_LABELS.md` rather than back here. And the dereference had
> to be built too: `review` resolved subjects with `get_nodes` alone, so a label
> id came back with a null preview meaning *not in this graph*. Subjects now
> carry `subject_kind`. **An id nobody dereferences is not an identity** — the
> lesson this entry is about, missed one layer up while fixing it.
...
### Issue 74 — a relation label is a string with no record, so nothing can describe it and nothing can decide about it — 🟢 RESOLVED 2026-08-28 (stages 1–3 built 2026-08-26/27/27; stage 4 settled by removing merging), raised 2026-08-24, supersedes #69
...
>
> 38 tests over both backends. Suite green at 2909. **FC1 was still live** at
> this point: it is stage 3, and needed stage 2 first so a verdict is made
> against a described vocabulary.

> **Stage 3 built 2026-08-27: FC1 is closed, and #69 with it.**
> `apply_reflection(relation_verdicts=[{pair, kind, verdict, because}])` records
> a pair judged `distinct` or `synonymous`; `RelationVerdict` is the append-only
> suppression index; `find_similar_relation_pairs` drops a judged pair. The
> journal row is `RELATION_VERDICT` over **both label record ids**, which is
> where #69 resolves — not by picking one of its four shapes but by the subject
> finally having an identity to name.
>
> **The measurement that motivates it, restated because it is easy to lose.**
> Getting the answer *right* is what caused the loop: accepting a merge makes one
> label stop existing, so accepting is self-suppressing and declining was not.
> The graph applied quiet pressure toward the answer that shortens the list, on a
> fresh agent each time who could not see the previous refusals. #64 measured the
...
>
> **Stage 4 settled 2026-08-28 by the user: `relation_merges` is removed and
> deprecation is not built** (§5). The stage does not ship, and nothing takes
> its place — a relation label is never rewritten. `reflect` still nominates,
> `relation_verdicts` records the judgment, `describe_relation` is what makes a
> vocabulary converge.
>
> **The sequencing worked exactly as this entry planned it**, which is the
> carry-forward: stage 3 had to land before any removal, because
> `similar_relations` nominations existed to feed `relation_merges` and would
> otherwise have fed nothing — FC1's treadmill still running, now to no
> destination. It landed 2026-08-27, the constraint discharged, and removal
> followed the next day.
>
> **What went with it**: the parameter on `apply_reflection` and the MCP tool;
> the `relations_consolidated` / `edges_relabeled` /
> `relation_descriptions_orphaned` response keys; `relabel_edges` from the
> protocol and all three backends, that being the mechanism rather than only the
> caller; and #81's residue, which closes because nothing strands a record any
> more. The fourth label write path §2.3 was corrected to name is a write path
> again no longer. 13 tests pin the absence at both layers, and the backend
> guard was falsified by putting the method back.
>
> **The argument, in one line:** the system had exactly one operation with no
> undo and it spent that on the least valuable thing in the graph. Dropping is
> reversible and building is not, so the gap deprecation would fill is left
> open — a `synonymous` verdict acts on nothing, and that is now permanent
...
> **Still open after #74 stage 3, 2026-08-27, and deliberately.** Stage 3 edited
...
second instance** — #74 stage 3 shipped 2026-08-27, so the system holds two
...
### Issue 81 — a relation merge strands the label record it merged away — 🟢 CLOSED 2026-08-28 by removing relation merging (partly fixed 2026-08-27, found the same day, from #74 stage 2)
...
and stage 3's `relation_verdicts` (all four now built). **There are four.**
...
**What was left was consequence 1, and it could not be fixed here.** Removing
the stranded record needed either a hard delete — which this system has exactly
one of, `reverse_merge_tx`, deliberately unreachable from any MCP tool — or a
`status` on the record, which is stage 4. So it was recorded as evidence for
stage 4 rather than a gap in stage 2: merging without a state model on the label
record strands records, a measured cost rather than a predicted one, and
whatever settled §5 would settle this.

**§5 settled it by removal, 2026-08-28.** Nothing rewrites a label, so nothing
strands a record: the entry closes without the residue being fixed, because the
operation that produced it no longer exists. Worth naming as an outcome in its
own right — **this entry was one of the two measured costs the removal was
argued from**, and a defect that can only be repaired by a mechanism nobody has
decided to build is evidence about the operation, not only about the gap. The
two halves fixed on 2026-08-27 went with it.
...
### Issue 82 — a malformed `apply_reflection` entry aborts a batch that is already half-written — 🟢 FIXED 2026-08-28 (raised the same day, from the #74 stage-3 read-surface review)
...
`batch_validation.py`, called as step 0 of `apply_reflection`: every entry is
checked before the first step writes, and a batch containing one that cannot be
applied at all is refused whole, with every problem listed at once rather than
the first. The behaviour is stated in `docs/REFLECTION.md` §6.

**The line the module draws is the design, and it is not "validate the
input".** It covers exactly what would otherwise *raise* from inside a loop — a
required key read with `spec["…"]`, an entry that is not the shape the loop
indexes into, and the values that are parsed or looked up in a closed set
(`because`, `at`, a `pair` of two). It covers none of what the graph can
evaluate and reject: an unknown id is still skipped, a stale kind still refused
into `*_refused`. Refusing a whole batch because one nominated pair had already
been judged would be this defect's mirror image, and `relation_verdicts.kind` is
the case that proves the distinction was needed — stage 3 refuses it per entry
**deliberately**, and promoting it here would have thrown away nine good
verdicts for an incomplete tenth. It is listed in `DELIBERATELY_PER_ENTRY` so
the exemption reads as a decision.

**The guard is an AST scan, for the reason the frame-edge guard is one.** A
required key added to a loop and not to `REQUIRED_KEYS` restores the defect
silently, and a guard whose reach is a list somebody maintains fails open. The
scan reads `apply_reflection`'s own subscripts and compares them against the
table in both directions: a key the loop reads and nothing declares fails, and a
key declared that the loop no longer reads fails too. Both were shown to fire.

**The wrapper was deleted rather than kept.** Its docstring justified it for
"callers with no report to file" and there were none, which is #74's own lesson
about measuring a feature before designing around it, one tier smaller. The five
call sites in two test modules now take `sweep_similar_relation_pairs(…).pairs`,
and the three docstrings that named it as the thing that re-derives per sweep
now name the sweep, which is what actually did it.

55 tests. Stubbing the gate turns 8 red; the drift guard was falsified in both
directions.
