...
| **next** | 68 (no retraction), or `REVIEW_MODE.md` step 4 | 68 is small and bounded — every `similarity` edge was written deliberately, so the population that could need retracting is tiny — and it is the third instance of #64's shape after #66's two. Step 4 threads `judge` through ingest and carries the per-graph require-a-judge setting (§3.3.1) — the bigger churn, and where the unreviewable priors are. Neither blocks the other |
