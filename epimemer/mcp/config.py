...
Factory functions create storage, embedding, and decomposition providers
based on the config.
"""
...
class ServerConfig(BaseModel):
    ...
    decomposition_provider: Literal["pydantic-ai", "mock"] = "pydantic-ai"
...
def load_config() -> ServerConfig:
    ...
    env_map = {
        ...
        "decomposition_provider": "EPIMEMER_DECOMPOSITION_PROVIDER",
        "decomposition_model": "EPIMEMER_DECOMPOSITION_MODEL",
...
def create_decomposition_provider(config: ServerConfig) -> DecompositionProvider:
    """Create a decomposition provider from config."""
    if config.decomposition_provider == "pydantic-ai":
        from epimemer.llm.pydantic_ai_provider import PydanticAIDecompositionProvider
        return PydanticAIDecompositionProvider(model=config.decomposition_model)
    else:
